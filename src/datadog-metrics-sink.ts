...
export class DatadogMetricsSink implements MetricsSink {
    ...
    private logger: Logger = createLogger("datadog-metrics");
    ...
    private async push(): Promise<void> {
        try {
            ...
            this.logger.error({err}, "error pushing datadog metrics")
        }
    }
    ...
    private submit(series: DatadogSeries[]): void {
        ...
        req.on('error', (err) => {
            this.logger.error({err}, "error submitting datadog metrics")
         })
        req.write(body)
        req.end()
    }
